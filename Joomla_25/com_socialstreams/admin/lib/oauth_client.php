...
*/
	Function CallAPI($url, $method, $parameters, $options, &$response)
	{
		...
		switch(intval($this->oauth_version))
		{
			...
			case 2:
				$oauth = null;
				$url .= (strcspn($url, '?') < strlen($url) ? '&' : '?').'oauth_token='.UrlEncode($this->access_token);
				break;
						...
						else
						{
							...
							$access_token = array(
								...
								'authorized'=>true
                                                        ...
                                                        /**
                                                         * @internal added the check for empty expires_in parameter to get around Linkedin sending 0 to indicate no expiry date
                                                         */
							if(IsSet($response['oauth_expires_in']) && !empty($response['oauth_expires_in']))
							{
