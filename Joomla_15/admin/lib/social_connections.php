...
class appsolFacebookApi extends AppsolSocialStreamsFacebook {
    ...
    function __construct() {
        ...
        $params = array(
            'appId' => $registry->getValue('socialstreams.facebook_' . $this->params['oauth']['apikey'], ''),
            'secret' => $registry->getValue('socialstreams.facebook_' . $this->params['oauth']['apisecret'], '')
        );
    ...
    function connect() {
        ...
        $registry = & JFactory::getConfig();
        // Get the user ID.
        $facebook_user_id = $this->getUser();
        try {
            $this->user = new appsolFacebookProfile();
            $user = $this->api('/' . $facebook_user_id);
    ...
    function get_request_url() {
        ...
        $url = $this->getLoginUrl(array(
            'redirect_uri' => JURI::base() . 'index.php?option=com_socialstreams&controller=config&task=setaccess&network=' . $this->params['network'],
...
class appsolTwitterApi extends TwitterOAuth {
    ...
    function get_request_url() {
        ...
        $request_token = $this->getRequestToken(JURI::base() . 'index.php?option=com_socialstreams&controller=config&task=setaccess&network=' . $this->params['network']);
        /* Save temporary credentials to session. */
        $session->set('twitter_oauth_token', $request_token['oauth_token'], 'socialstreams');
        $session->set('twitter_oauth_token_secret', $request_token['oauth_token_secret'], 'socialstreams');
        if ($this->lastStatusCode() == 200)
