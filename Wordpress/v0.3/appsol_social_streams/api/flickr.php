...
class flickrHelper {
    ...
    public static function setup($userid = null) {
        ...
        $errorLog->addEntry(array('status' => 'DEBUG', 'comment' => 'flickrHelper::setup'));
        self::$client = new SocialStreamsFlickr();
        self::$client->user = $userid;
        self::$client->debug = self::$debug;
        self::$client->server = self::$server;
        self::$client->scope = self::$scope;
        self::$client->redirect_uri = JURI::base() . 'index.php?option=com_socialstreams&task=socialstream.setauth&network=flickr';

        $jparams = JComponentHelper::getParams('com_socialstreams');

        if (strlen($jparams->get('flickr_appkey')) == 0 || strlen($jparams->get('flickr_appsecret')) == 0)
            return false;
        self::$client->client_id = $jparams->get('flickr_appkey');
        self::$client->client_secret = $jparams->get('flickr_appsecret');
            ...
            if ($success = self::$client->Process()) {
                if (strlen(self::$client->access_token)) {
                    $success = self::$client->CallAPI(
                            'https://graph.flickr.com/me', 'GET', array(), array('FailOnAccessError' => true), $user);
                }
            }
...
class SocialStreamsFlickr extends SocialStreamsApi {

    private $api_url = 'https://graph.flickr.com/';
    ...
    public function getProfile($id = 'me') {
        ...
        if ($success) {
            $profile = new SocialStreamsFlickrProfile();
    ...
    public function getConnectedProfiles() {
        jimport('joomla.error.log');
        $errorLog = & JLog::getInstance();
        $errorLog->addEntry(array('status' => 'DEBUG', 'comment' => 'SocialStreamsFlickr::getConnectedProfiles'));
        $my_friends = array();
        if (strlen($this->access_token))
    ...
    public function getItems() {
        jimport('joomla.error.log');
        $errorLog = & JLog::getInstance();
        $errorLog->addEntry(array('status' => 'DEBUG', 'comment' => 'SocialStreamsFlickr::getItems'));
        $my_feed = array();
        if (strlen($this->access_token))
        ...
        if ($success) {
            foreach ($feed->data as $post) {
                if (isset($post->privacy) && $post->privacy->value == 'EVERYONE') {
                    $item = new SocialStreamsFlickrItem();
                    $item->setUpdate($post);
...
/**
 * Coomon interface to a Flickr Profile object 
 */
class SocialStreamsFlickrProfile extends SocialStreamsProfile {

    public function __construct($wraptag = 'li') {
        $this->network = 'flickr';
        $this->nicename = 'Flickr';
    ...
    public function setProfile($profile) {
        jimport('joomla.error.log');
        $errorLog = & JLog::getInstance();
        $errorLog->addEntry(array('status' => 'DEBUG', 'comment' => 'SocialStreamsFlickrProfile::setProfile'));
//        $errorLog->addEntry(array('status' => 'DEBUG', 'comment' => print_r($profile, true)));
        $profile_image = 'http://graph.flickr.com/' . $profile->id . '/picture?type=square';
...
class SocialStreamsFlickrItem extends SocialStreamsItem {
    ...
    function setUpdate($post, $fb_user = null) {
        jimport('joomla.error.log');
        $errorLog = & JLog::getInstance();
        $errorLog->addEntry(array('status' => 'DEBUG', 'comment' => 'SocialStreamsFlickrItem::setUpdate'));
//        $errorLog->addEntry(array('status' => 'DEBUG', 'comment' => 'Post -> ' . print_r($post, true)));
        ...
        if (isset($post->item)) {
            ...
            if (isset($post->profile)) {
                $this->profile = new SocialStreamsFlickrProfile();
...
//        jimport('joomla.error.log');
//        $errorLog = & JLog::getInstance();
//        $errorLog->addEntry(array('status' => 'DEBUG', 'comment' => 'appsolFlickrItem::setUpdateActions'));
